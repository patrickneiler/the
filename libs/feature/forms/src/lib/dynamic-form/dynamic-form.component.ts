...
export class DynamicFormComponent implements OnInit, OnChanges, OnDestroy {
  ...
  public get color(): string {
    return `var(--ion-color-${this._color})`;
  }
  @Input()
  ...
  ngOnChanges(changes: SimpleChanges) {
    changes['color']
      ? (this._color = changes['color'].currentValue)
      : this._color;
  }
  ...
  private control = (field: Field): FormControl => {
    return this.fb.control(field.value ? field.value : '', field.validator);
  };

  private patchValue = ([form, data]: [FormGroup, any]): void => {
    data
